...
with open('2564-12-01AM.csv', 'w', newline='') as f:
                fw = csv.writer(f)
...
# frame dimensions
vs = cv2.VideoCapture(args["input"]+'.mp4' if args["input"] else 0)
writer = None
(W, H) = (None, None)
		...
		for i in idxs.flatten():
			...
			for (x,y) in detec:
                            ...
                            if y<(roi+offset) and y>(roi-offset):
                                ...
                                print("Vehicle type",vehicle,"date",textd[0:10],"time", textt[0:2]+':'+textt[2:4]+':'+textt[4:6])

                                with open('2564-12-01AM.csv', 'a', newline='') as f:
                                        fw = csv.writer(f)
                                        fw.writerow([counter, vehicle, textd[0:10], textt[0:2]+':'+textt[2:4]+':'+textt[4:6]])
vs.release()
